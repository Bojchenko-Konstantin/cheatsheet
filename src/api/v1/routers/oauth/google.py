...
from src.infrastructure.exceptions import GoogleOAuthException
...
async def login(oauth_provider_service: GoogleOAuthServiceDep):
    ...
    response.set_cookie(key="google_oauth_state", value=state, **cookie_params)
    response.set_cookie(
        key="google_oauth_code_verifier", value=code_verifier, **cookie_params
    )
...
async def callback(
    request: Request,
    oauth_use_case: GoogleOAuthUseCaseDep,
    params: Annotated[OAuthCallbackParams, Query()],
):
    state_from_cookie = request.cookies.get("google_oauth_state")
    code_verifier_from_cookie = request.cookies.get("google_oauth_code_verifier")
    ...
    try:
        token_pair = await oauth_use_case.authenticate(
            ...
            oauth_service=OAuthService.GOOGLE,
        )
    ...
    response.delete_cookie("google_oauth_code_verifier", path="/")

    return response


@router.delete("auth/google/connections")
async def unlink(
